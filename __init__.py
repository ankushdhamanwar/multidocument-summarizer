...
def gen_sim_matrix(text1, text2):
    q1_sents_tok = sent_tokenize(text1)
    q2_sents_tok = sent_tokenize(text2)
...
# print(df.head())
    ...
    for k, v in sorted_x[:leng]:
        ...
        if q1score > q2score:
            summ.append(q1_sents_tok[k[0]])
        else:
            summ.append(q2_sents_tok[k[1]])
...
def index():
    if request.method == 'POST':
        ...
        result1=request.form.get('text1')
        result2=request.form.get('text2')
        res = gen_sim_matrix(result1, result2)
        return render_template('index.html',result={'result':res,'res1':result1,'res2':result2})
    result1 = ""
    result2 = ""
    res = ""
    return render_template('index.html',result={'result':res,'res1':result1,'res2':result2})
